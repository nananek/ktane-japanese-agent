...
- 以下は `llm/solvers.py` が解析し、表の照合・経路探索・ステージをまたぐ記録をコード側で確定させる
  ...
  - `whos-on-first.md` (`solve_whos_on_first`) — ステップ1の `| 表示語 | N行目-左/右 |` とステップ2の
    `| 読んだ単語 | 優先順位 (カンマ区切り) |` の表。ボタンは左上・右上・左中・右中・左下・右下の順で受け取り、
    同じ読みで漢字の違う表示語 (かい: 解/回/下位/快/開 など、`WOF_HOMOPHONES`) は、LLMが漢字を確定させていなければ
    コード側で候補を広げ、押すボタンが変わる場合だけ漢字を聞き返す。ほかのボタンと別の語群の文字や「不明」の位置も、
    答えが変わる場合だけ聞き返す
  - `complicated-wires.md` (`solve_complicated_wires`) — `| あり/なし | ×4 (赤・青・★・LED) | 指示コード |` の判定表と
    `| コード | 意味 |` の表。Defuserが分けて言う本数・色・LED・★を爆弾の状態に積み上げ、足りない項目だけを聞き返す。
    指示コードの条件 (シリアル偶数・パラレルポート・バッテリー) はソルバーが解決し、切るワイヤだけを伝える
  - `simon-says.md` (`solve_simon_says`) — `## シリアルナンバーに母音が含まれて(いる|いない)場合` ごとに
    `| 点滅した色 | ミスなし | 1ミス後 | 2ミス後 |` の表。光った色の並びを積み上げ、記録済みのミス数で押す色を決める
...
ワイヤ・ボタンのソルバーが爆弾の情報を聞き返した場合は、答えが `update_bomb_info` で記録された時点で同じ入力のまま
判定し直し、LLMにソルバーを呼び直させる往復も省く (聞き返した次の発言でだけ行う)。
...
- `/ktane-newbomb` — 新しい爆弾用に会話と爆弾情報をリセットする。実行した人がbotと同じボイスチャンネルに
  ...
  (`force: True` で強制実行)。Defuserが「ゲームを中断します」と言った場合も終了 (中断) として記録する
...
Discord・Whisper・VOICEVOXを通さずに、LLMとソルバーの応答をテキストで確かめられる。
`.env` のLLM設定 (`LLM_*`) と `knowledge/` だけあれば動き、`DISCORD_TOKEN` やGPUは不要。

```bash
python3 web.py   # http://127.0.0.1:8080
```

- botとは別プロセスで、会話と爆弾の状態はブラウザのタブごとに持つ。通話中のセッションには影響しない
  (サーバーのメモリ上にだけあり、`web.py` を再起動すると消える)
- 言いよどみの無視・聞き返し・ゲーム終了後の扱い・`新しい爆弾` の確認はbotと同じ (`conversation.py` を共有)
- ツール呼び出しは引数と結果を折りたたんで表示する。LLMが発言をどう構造化したかの確認に使う
- 認証がないため既定では `127.0.0.1` だけで待ち受ける。変える場合は `WEB_HOST` / `WEB_PORT`
