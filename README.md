...
- `keypads.md` — 列一覧を `- 列N (上から押す順): Ϙ(見た目, 28-balloon) → ...` の形式で書く。
  `llm/solvers.py` がこれを解析し、`solve_keypad` ツールで列の照合と押す順番をコード側で確定させる
...
- AI発話中にDefuserが話しかけてきた場合の割り込み (barge-in) は未対応。応答生成や読み上げの間に
  届いた発話はまとめて次の1回の応答で扱い、応答生成中に届いた場合はまとめて作り直す。
